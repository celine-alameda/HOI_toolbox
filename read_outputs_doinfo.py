...
save_name = config["input"].split(".")[0] + "_dO"
do_info_dict = load_obj(save_name)

print("dOinfo readout, object {}".format(save_name))
print(do_info_dict)

n_variables = len(variable_names)
output_data = []
for target_variable in range(n_variables):
    for nplet_size in range(2, n_variables + 1):
        ...
        if "index_red" in do_info_dict[target_variable][nplet_size]:
            red_indices = do_info_dict[target_variable][nplet_size]["index_red"]
            red_values = do_info_dict[target_variable][nplet_size]["sorted_red"]
            red_significancies = do_info_dict[target_variable][nplet_size]["bootsig_red"]
        ...
        if "index_syn" in do_info_dict[target_variable][nplet_size]:
            syn_indices = do_info_dict[target_variable][nplet_size]["index_syn"]
            syn_values = do_info_dict[target_variable][nplet_size]["sorted_syn"]
            syn_significancies = do_info_dict[target_variable][nplet_size]["bootsig_syn"]
    ...
    output_file_name = save_name + "_tv_" + str(target_variable) + ".xlsx"
