        ...
        for nplet_size in tqdm(range(3, maxsize + 1), disable=True):
            ...
            for combination_index in tqdm(range(n_combinations), desc="Inner loop, computing O", leave=False):
                if higher_order:
                    ...
                    o_array = o_information_boot(X, range(n_observations), comb - 1, estimator)
                    ...
                    o_array[combination_index] = o_information_boot(X, range(n_observations), comb - 1, estimator)
