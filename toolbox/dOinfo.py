...
def exhaustive_loop_lagged(ts, config):
    ...
    print("Timeseries details - Number of variables: ", str(n_variables), ", Number of timepoints: ", str(n_timepoints))
        ...
        for index_target in tqdm(range(n_variables), disable=True):
            ...
            for isize in tqdm(range(2, maxsize + 1), disable=True):
                ...
                for icomb in tqdm(range(ncomb), desc="Inner loop, computing O", leave=False):
                    if higher_order:
                        comb = H.nextchoose()
                        Osize = o_information_lagged_boot(t, X, modelorder, np.arange(n_timepoints), 0, var_arr[comb - 1] - 1,
                                                          estimator)
                        ...
                        comb = C[icomb, :]
                        Osize[icomb] = o_information_lagged_boot(t, X, modelorder, np.arange(n_timepoints), 0, comb - 1, estimator)
                if higher_order:
                ...
                if Osort_pos.size != 0:
                    ...
                    for isel in range(n_sel):
                        if higher_order:
                            ...
                            f = lambda xsamp: o_information_lagged_boot(t, X, modelorder, xsamp, chunklength,
                                                                        var_arr[indvar - 1] - 1, estimator)
                        else:
